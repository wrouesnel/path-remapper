...
static asmlinkage int (*orig_get_cmdline)(struct task_struct *task,
...
static notrace bool is_targeted_prefix(const char *filepath) {
    ...
    if (!str_has_prefix(filepath, FIXED_PREFIX)) {
        return false;
    }

    // Placeholder - check for the suffix
    if (!strends(filepath, FIXED_SUFFIX)) {
        return false;
    }
    ...
    int res = orig_get_cmdline(current, cmdline, PAGE_SIZE);
    if (res == 0) {
        kfree(cmdline);
        return false;
    }
    ...
    // Log the filename
    pr_info("remap: %s->%s\n", filepath, FIXED_REPLACEMENT);

    kfree(cmdline);
    return true;
}

static notrace asmlinkage long hook_get_cmdline(struct task_struct *task,
						char *buffer, int buflen)
{
...
static notrace asmlinkage ssize_t hook_pread64(const struct pt_regs *regs)
{
	...
	filename = file->f_path.dentry ? file->f_path.dentry->d_name.name :
					 NULL;
...
static notrace asmlinkage ssize_t hook_read(const struct pt_regs *regs)
{
	...
	// allocate a buffer to resolve the file path into
	char *pathbuffer = (char *)kzalloc(PATH_MAX, GFP_KERNEL);
	if (!pathbuffer) {
        fput(file);
        return orig_read(regs);
    }

    char* filepath = dentry_path_raw(file->f_path.dentry, pathbuffer, PATH_MAX-1);
    if (IS_ERR(filepath)) {
        kfree(pathbuffer);
        fput(file);
        return orig_read(regs);
    ...
    if (is_targeted_prefix(filepath)) {
        // TODO
    }
